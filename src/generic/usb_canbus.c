...
static void
usb_do_xfer2(void *data, uint_fast8_t size, uint_fast8_t flags, void (*complete)())
{
    for (;;) {
...
{
    usb_do_xfer2(&device_bittiming, sizeof(device_bittiming), UX_READ, gs_breq_bittiming_complete);
}
...
{
    usb_do_xfer2(&device_mode, sizeof(device_mode), UX_READ, gs_breq_mode_complete);
}
...
usb_ep0_task(void)
{
    if (!sched_check_wake(&usb_ep0_wake))
        return;
    if (usb_xfer_flags)
        usb_do_xfer2(usb_xfer_data, usb_xfer_size, usb_xfer_flags, usb_xfer_complete);
    else
        usb_state_ready();
}
