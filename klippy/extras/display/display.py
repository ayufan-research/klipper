...
class PrinterLCD:
    def __init__(self, config):
        ...
        # Brightness control
        self.dim_value = config.getfloat('dim_value', 0.1, minval=0., maxval=1.0)
        self.bright_value = config.getfloat('bright_value', 0.7, minval=self.dim_value, maxval=1.0)
        ...
        if brightness_pin:
            ...
            cycle_time = config.getfloat('brightness_cycle_time', 0.001, above=0.)
            ...
            self.mcu_brightness_pin.setup_start_value(self.dim_value, self.dim_value)
        # Screen updating
    ...
    def dim_brightness_event(self, eventtime):
        ...
        pt = self.mcu_brightness_pin.get_mcu().estimated_print_time(systime + BRIGHTNESS_CHANGE_DELAY)
    ...
    def request_redraw(self):
        ...
        if self.mcu_brightness_pin:
            ...
            self.reactor.update_timer(self.dim_update_timer, systime + self.dim_timeout)
