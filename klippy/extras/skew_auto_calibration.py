...
class SkewAutoCalibration:
    ...
    def _find_gantry_corners(self, gcmd):
        ...
        for corner_pt in self.gantry_corners:
            ...
            gcmd.respond_info("%s: Found at %s" % (self._vect_round(corner_pt), self._vect_round(center_pt)))
    ...
    def _find_circle_center_n(self, gcmd, center):
        ...
        for i in range(self.iterations):
            ...
            gcmd.respond_info("%s: Iteration %d: %s / %.3f / %.3f" % (
                self._vect_round(center), i, self._vect_round(pt), radius, accuracy
            ))
    ...
    def _find_circle_center(self, gcmd, center):
        ...
        for i in range(len(edges)-1):
            ...
            length = self._vect_len2(e1, e2) + self._vect_len2(e2, e3) + self._vect_len2(e1, e3)
    ...
    def _find_edges(self, gcmd, p1):
        ...
        for dir_pt in dirs:
            ...
            if edge_pt:
                ...
                gcmd.respond_info("edge not found for %s-%s" % (self._vect_round(p1), self._vect_round(p2)))
    ...
    def _endstop_state_at(self, pos):
        ...
        toolhead = self.printer.lookup_object('toolhead')
        toolhead.manual_move([None, None, self.z_position + self.z_hop], self.z_speed)
        toolhead.manual_move(pos, self.speed)
        toolhead.manual_move([None, None, self.z_position], self.z_speed)
        #TODO: is this needed? will `request_status` request at a particular moment
        #toolhead.wait_moves()
